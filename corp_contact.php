<?php
    ...
    $to         = 'ananth.raysiti@gmail.com';

    $email_template = 'corp_contact.html';

    $requirement = strip_tags($_POST['requirement']);
    $email      = strip_tags($_POST['email']);
    $phone      = strip_tags($_POST['phone']);
    $name       = strip_tags($_POST['name']);
    $company    = strip_tags($_POST['company']);
    $designation = strip_tags($_POST['designation']);
    $quntity    = strip_tags($_POST['quantity']);
    ...
    if(empty($requirement)){
        ...
        $result = array( 'response' => 'error', 'empty'=>'requirement', 'message'=>'<strong>Error!</strong>&nbsp; Mobile no. is empty.' );
        echo json_encode($result );
        die;
    }
    ...
    if ( mail( $to, $requirement, $contents, $headers ) ) {
